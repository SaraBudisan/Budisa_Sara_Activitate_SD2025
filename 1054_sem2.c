...
struct Masina initializare(int id, int anFabricatie, const char* sofer, float kilometriiParcursi, char initialaProducator) {
	struct Masina m;//declaram o masina m pe care o initializam
	m.id = id;
	m.anFabricatie = anFabricatie;
	if (sofer != NULL)
	{
		m.sofer = (char*)malloc(strlen(sofer) + 1); //ii alocam memorie dinamic pentru a evita memory leak
...
void afisare(struct Masina m) { //afisam  m 
	printf("ID: %d \n", m.id);
...
//returneaza masina care are mai multi km decat un prag impus de noi
...
//returneaza prima masina pe care o conduce un anume sofer
...
int main() {
	...
	dezalocare(&vectorNou, &nrElementeCopiate);//nu uita de dezalocare!!!
